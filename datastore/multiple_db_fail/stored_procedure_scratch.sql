...
-- Removed in favor of dumping.
/*
-- --------------------------------------------------------------------------------
-- Routine to copy subject_template schema to a new schema for each new subject.
-- --------------------------------------------------------------------------------
DELIMITER $$

DROP PROCEDURE IF EXISTS `create_subject` $$
CREATE PROCEDURE `create_subject`(IN sub_name VARCHAR(255))
BEGIN
    DECLARE db_name VARCHAR(255);   -- new schema name for this subject.
    DECLARE done INT DEFAULT FALSE; -- used to stop cursor loops
    
    DECLARE tbl_name VARCHAR(255);  -- specific table name
    DECLARE curs1 CURSOR FOR        -- cursor to get table names
        SELECT TABLE_NAME
        FROM information_schema.TABLES
        WHERE TABLE_SCHEMA LIKE 'eerat_subject_template';
    
    DECLARE trig_name VARCHAR(255);     -- trigger name
    DECLARE trig_timing VARCHAR(255);   -- trigger time
    DECLARE trig_stmnt VARCHAR(255);    -- actual trigger text. Needs longer than 255.
    DECLARE curs2 CURSOR FOR
        SELECT TRIGGER_NAME, EVENT_OBJECT_TABLE, ACTION_STATEMENT, ACTION_TIMING
        FROM information_schema.TRIGGERS
        WHERE TRIGGER_SCHEMA LIKE 'eerat_subject_template';
    
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE; -- Common to all cursors.
    
    
    SELECT CONCAT('eerat_subject_', sub_name) INTO db_name;
    
    -- DROP DATABASE
    SET @stmnt1_txt = CONCAT('DROP DATABASE IF EXISTS ',db_name);
    PREPARE stmnt1 FROM @stmnt1_txt;
    EXECUTE stmnt1;
    DEALLOCATE PREPARE stmnt1;
    
    -- CREATE DATABASE db_name CHARACTER SET UTF8 COLLATE utf8_general_ci;
    SET @stmnt2_txt = CONCAT('CREATE DATABASE ',db_name,' CHARACTER SET UTF8 COLLATE utf8_general_ci');
    PREPARE stmnt2 FROM @stmnt2_txt;
    EXECUTE stmnt2;
    DEALLOCATE PREPARE stmnt2;
    
    -- CREATE TABLE db_name.tbl_name LIKE eerat_subject_template.tbl_name;
    OPEN curs1;
    loop1: LOOP
        FETCH curs1 INTO tbl_name;
        IF done THEN
            LEAVE loop1;
        END IF;
        SET @stmnt3_txt = CONCAT('CREATE TABLE ',db_name,'.',tbl_name,' LIKE eerat_subject_template.',tbl_name);
        PREPARE stmnt3 FROM @stmnt3_txt;
        EXECUTE stmnt3;
        DEALLOCATE PREPARE stmnt3;
    END LOOP loop1;
    CLOSE curs1;
    
    -- SELECT TRIGGER_NAME, EVENT_OBJECT_TABLE, ACTION_STATEMENT, ACTION_TIMING FROM information_schema.TRIGGERS WHERE TRIGGER_SCHEMA LIKE 'eerat_subject_template';
    SELECT FALSE INTO done;
    OPEN curs2;
    loop2: LOOP
        FETCH curs2 INTO trig_name, tbl_name, trig_stmnt, trig_timing;
        IF done THEN
            LEAVE loop2;
        END IF;
        -- SET @stmnt4_txt = CONCAT('
        
    END LOOP loop2;
    CLOSE curs2;
    
    SELECT db_name; -- to clear a warning generated by the last empty fetch.
END $$
