...
%For each subject,
for ss=2:2%length(subject_info)
    si=subject_info(ss);
    ...
    for dd=1:length(si.day)
        ...
        for ff=1:length(si.day(dd).file_names)
            ...
            elseif n_channels==length(channel_labels{2})
                ...
                fs=1200;
            elseif n_channels==14
                %1:trigger, 2-11:EEG, 13-14:EMG
                ch_l={'trigger','FC3','C5.','C3.','C1.','CP3','FC4','C2.','C4.','C6.','CP4','EMG1','EMG2','EMG3'};
                fs=1200;
            else
...
%             task_stops=(find(diff([0;trig_sig])<0));
            ...
            base_sig=nan(fs,n_trials,n_freqs);
            task_sig=nan(fs,n_trials,n_freqs);
            for tt=1:n_trials
                base_sig(:,tt,:)=sig(task_starts(tt)-2*fs:task_starts(tt)-fs-1,:);
                task_sig(:,tt,:)=sig(task_starts(tt)+fs:task_starts(tt)+2*fs-1,:);
            end
