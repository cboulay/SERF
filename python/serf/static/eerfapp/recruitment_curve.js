$(document).ready(function () {
    ...
    $.get("http://127.0.0.1:8000/eerfapp/detail_types/", {}, function(response){
    ...
    $.get("http://127.0.0.1:8000/eerfapp/feature_types/", {}, function(response){
    	response = JSON.parse(response);
    	for (var j=0; j<response.length; j++){
			var new_option = $('<option value='+response[j]+'>'+response[j]+'</option>');
    ...
    var plot_xy = function(){
    	$.get("http://127.0.0.1:8000/eerfapp/get_xy/"
    ...
    var recalculate = function() {
    	if ($('input.recalculate')[0].checked){
    		$.get("http://127.0.0.1:8000/eerfapp/subject/" + subject_pk + "/recalculate_feature/" + $('select.y_name').val()
	    	, {}, function(result){
	    		plot_xy();
	    	});
    	} else {plot_xy();}
